...
router.post('/add',auth.requireLogin, function (req, res, next) {
    ...
    req.checkBody('category','必须指定文章类型').notEmpty();
...
router.get('/delete/:id', auth.requireLogin,function (req, res, next) {
  ...
  Post.remove({_id:req.params.id}).exec(function(err,rowsRemoned){
    if(err){
      return next(err);
    }
    if(rowsRemoned){
      req.flash('sucess','文章删除成功');
    }else{
      req.flash('sucess','文章删除失败');
    }
    res.redirect('/admin/posts');
  });
});
